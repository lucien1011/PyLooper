...
__global__ void calculate(float *dest, float *xs_weight, float *selection_weight, float* dataMCWeight, float* pileupWeight)
{
  const int i = threadIdx.x + blockDim.x * blockIdx.x;
  dest[i] = xs_weight[i]*selection_weight[i]*dataMCWeight[i]*pileupWeight[i];
}
""")
    ...
    def analyze(self,data,dataset,cfg):
        if not dataset.skip_weight:
            if dataset.isMC:
                ...
                cfg.collector.event_weight = np.ones(cfg.ibatch)
            else:
                cfg.collector.event_weight = np.ones(data["genWeight"].shape) * cfg.collector.selection_weight
